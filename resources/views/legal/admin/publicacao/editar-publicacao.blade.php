    ...
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form action="{{ route('edit-publicacao', $publicacao->id) }}" method="POST" class="needs-validation" enctype="multipart/form-data" novalidate>
            ...
            <div class="mb-1">
                ...
                <input 
                    ...
                    id="fp-default" 
                    name="data"
                    value=@if (old('data')) {{ old('data') }} @else {{ $publicacao->data }} @endif
                ...
                <select class="form-control select2" name="empresa_id" id="empresa_id" required>
                  ...
                  @if (isset($empresas))
                      @foreach ($empresas as $empresa)
                        <option value="{{$empresa->id}}" @if ($empresa->id == old('empresa_id')) selected @else @if ($empresa->id == $publicacao->empresa_id) selected @endif @endif>{{$empresa->nome}}</option>    
                      @endforeach
                  @endif
                </select>
                ...
                <select class="form-control select2" name="tipo_id" id="tipo_id" required>
                  ...
                  @if (isset($tipos))
                      @foreach ($tipos as $tipo)
                        <option value="{{$tipo->id}}" @if ($tipo->id == old('tipo_id')) selected @else @if ($publicacao->tipo_id == $tipo->id) selected @endif @endif>{{$tipo->descricao}}</option>    
                      @endforeach
                  @endif
                </select>
            ...
            <div class="mb-1">
              ...
              <input
                type="url"
                name="url"
                value="@if (old('url')) {{ old('url') }} @else {{ $publicacao->url }} @endif"
                id="basic-addon-name"
                class="form-control"
            ...
            <div class="mb-1">
              ...
              <select class="select2 form-select" name="inativo" id="inativo">
                  ...
                  <option value="1" @if (old('inativo') == 1) selected @else @if ($publicacao->inativo == 1) selected @endif @endif>Inativo</option>
              </select>
              <div class="invalid-feedback">Por favor, informe a Situação.</div>
            </div>

            <div class="mb-1">
                <label for="anexo" class="form-label">Arquivo<p class="fst-italic">
